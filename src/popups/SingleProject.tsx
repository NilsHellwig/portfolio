...
const SingleProject: React.FC<SingleProjectProps> = ({ showSingleProjectFct, projectId }) => {
  ...
  return (
    <motion.div
      className={`white-case fixed top-0 left-0 right-0 bottom-0 max-w-[800px] w-[100%] xmd:my-auto xmd:h-3/4 my-auto xmd:mx-auto z-50 bg-white dark:bg-zinc-900 xmd:rounded-xl xmd:border xmd:border-zinc-300 xmd:dark:border-zinc-700 p-6 pb-12 ${
    ...
    >
      ...
      <motion.div variants={itemVariants} className="inner-content flex flex-col gap-6">
        <div className="flex sm:flex-row gap-4 my-8 items-center flex-col justify-center sm:justify-normal mt-16">
          <div className="bg-white dark:bg-zinc-800 rounded-[30px] w-28 h-28 border border-zinc-400 dark:border-zinc-600 flex justify-center items-center">
          ...
          <div className="flex flex-col text-center sm:text-left gap-1 items-center sm:items-start">
            <span className="font-bold text-2xl dark:text-white">{project?.title}</span>
            <span className="text-zinc-400">{project?.subtitle}</span>
            <div className="border border-zinc-200 dark:border-zinc-700 rounded-full px-2 py-1 w-fit">
              <span className="text-sm dark:text-zinc-300">{project?.type}</span>
            </div>
          </div>
        </div>
        <div>
          <h3 className="font-bold text-center dark:text-white">Gallery</h3>
          <div className="bg-zinc-100 dark:bg-zinc-800 rounded-xl border border-zinc-300 dark:border-zinc-700 p-4 mt-4 flex flex-col justify-between h-[580px]">
        ...
        <div>
          <h3 className="font-bold text-center dark:text-white">Programming Languages</h3>
        ...
        <div>
          <h3 className="font-bold text-center mb-4 dark:text-white">Links</h3>
          <div className="flex flex-col gap-4">
            {project?.links?.map((link, index) => {
              return (
                <Tooltip text={link.url} key={index}>
                  <div
                    className="p-3 border border-zinc-200 dark:border-zinc-700 rounded-xl flex justify-between hover:bg-zinc-100 dark:hover:bg-zinc-800 cursor-pointer items-center"
                    onClick={() => {
